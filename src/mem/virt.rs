...
pub fn virt_read(cr3: Address, gva: Address, buf: &mut Vec<u8>, sz: usize) -> Result<(), VirtMemError> {
    let gpa = virt_translate(cr3, gva)?;
    Ok(())
}

pub fn virt_write(cr3: PhyAddress, gva: Address, buf: &[u8]) -> Result<(), VirtMemError> {
    let gpa = virt_translate(cr3, gva)?;
    Ok(())
}

pub fn virt_translate(cr3: PhyAddress, gva: Address) -> Result<PhyAddress, VirtMemError> {
    let pdpte_addr = cr3 + pml4_index(gva) * 8;
    let pdpte = phy_read_u64(pdpte_addr);

    let (pdpt_base, pml4e_flags) = base_flags(pdpte);
    ...
    let (pte_paddr, pte_flags) = base_flags(pte);
