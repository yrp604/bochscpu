...
unsafe fn mem() -> &'static mut FastMap64<PhyAddress, *mut u8> { unsafe {
    &mut (*(MEM.0.get()))
}}

pub unsafe fn resolve_hva(gpa: PhyAddress) -> *mut u8 { unsafe {
    let (page, off) = page_off(gpa);
    (*(mem().get(&page).unwrap())).add(off)
}}

pub unsafe fn resolve_hva_checked(gpa: PhyAddress) -> Option<*mut u8> { unsafe {
    let (page, off) = page_off(gpa);
...
pub unsafe fn page_remove(gpa: PhyAddress) { unsafe {
    let (page, _) = page_off(gpa);
    mem().remove(&page);
}}
